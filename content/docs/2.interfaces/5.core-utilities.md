...
- `implementation`: The implementation of the interface

#### Return Value

Returns an object containing the two parameters:
...
ImplementInterface(userInterface, userImplementation);

// Alternatively, in a module's construct function:
export async function construct() {
  // Import the interface declaration and implementation
  // You can also pass the imports directly to ImplementInterface
  ImplementInterface(
    await import('@ajs.local/interfaces/users/1'),
    await import('./implementations/users/1')
  );
}
```
