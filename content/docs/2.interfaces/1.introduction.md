...
**Flexible Implementations**: When implementing interface functions, you can write either synchronous or asynchronous code. The framework automatically handles both—use `async` only where you actually need it.

```typescript
// Both work seamlessly
export function format(input: string): string { ... }  // Sync
export async function save(data: Data): Promise<void> { ... }  // Async
```
::
