...
| `--interactive` | -i    | Start an interactive REPL with the project    | false   | -                     |
| `--verbose`     | -v    | Enable verbose logging for specific channels  | -       | -                     |
...
ajs project run --verbose=loader.git,loader.package
ajs project run --interactive
```

::tip{icon="i-ph-lightbulb"}
The REPL is especially useful during development. You can inspect module state, test functions interactively, and debug issues without adding logs.
::

::note{icon="i-ph-info"}
See the [Logging documentation](/docs/interfaces/logging) for complete channel filtering configuration and examples.
::
